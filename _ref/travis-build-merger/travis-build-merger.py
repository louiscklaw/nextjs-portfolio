  ...
  with lcd(TEMP_DIR), settings(warn_only=True):
    with( shell_env( GIT_COMMITTER_EMAIL='travis@travis', GIT_COMMITTER_NAME='Travis CI' ) ):
      ...
      if result_to_check.failed:
        ...
        slack_message('merging BUILD{} from {} `{}` to `{}` done\n `{}`'.format(TRAVIS_BUILD_NUMBER, GITHUB_REPO, TRAVIS_BRANCH, BRANCH_TO_MERGE_INTO, TRAVIS_COMMIT_MESSAGE), '#travis-build-result')
